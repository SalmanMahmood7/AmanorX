...
function ResultBody({ result }) {
  return (
    <>
      <span className="font-mono text-xs font-medium tracking-wide text-gold-600 uppercase">
        {result.type}
      </span>
            ...
            <Reveal key={`${result.type}-${result.title}`}>
              {result.external ? (
                <a
                  ...
                  className="group block border border-t-2 border-navy-900/15 border-t-gold-500 bg-white p-6 transition-colors duration-300 hover:bg-navy-50 sm:p-7"
                >
