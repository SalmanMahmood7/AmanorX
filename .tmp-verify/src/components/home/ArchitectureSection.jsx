...
function TierConnector() {
  return (
    <div aria-hidden="true" className="flex flex-col items-center">
      <span className="h-4 w-px bg-gold-500/60 sm:h-5" />
      <span className="h-1.5 w-1.5 rotate-45 border border-gold-400 bg-navy-950" />
      <span className="h-4 w-px bg-gold-500/60 sm:h-5" />
    </div>
  );
}
...
function TierRow({ tier, scale }) {
  return (
    <div className="relative h-full overflow-hidden rounded-lg border border-white/10 bg-white/[0.04] p-6 transition-colors duration-300 hover:border-gold-500/40 hover:bg-white/[0.06] sm:p-8">
      <span
        aria-hidden="true"
      ...
      <div className="relative lg:grid lg:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)] lg:gap-12">
        <div>
          <div className="flex items-center gap-3">
            <span className="font-mono text-xs font-medium tracking-wide text-gold-400 uppercase">
              Tier {tier.tier}
            </span>
