...
// Live companies shown on the homepage portfolio logo wall (excludes
// entries flagged `onHomepageWall: false` in the registry).
export function getHomepageWallCompanies() {
  return getLivePortfolioCompanies().filter(
    (company) => company.onHomepageWall !== false
  );
}
